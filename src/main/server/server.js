...
function start() {
	...
	O.i('Using reappt info: ' + __diffHost + ':' + __diffPort + ' usingSSL: '
			+ __diffSSL + ' principal: ' + __diffPrincipal);

	__server = http.createServer(onRequest).on('close', onClose).on(
			'clientError', onClientError).listen(__serverPort);
	...
	diffusion.connect({
		...
		port : __diffPort,
		ssl : __diffSSL,
		principal : __diffPrincipal,
		credentials : __diffPassword
	}).then(
		function(session) {
			O.i('Connected to Diffusion with sessionID : ' + session.sessionID);
			session.topics.remove('DevelopDenver');
			__diffusion = session;
		},
		function(err) {
			O.e("Error with Diffusion connection :'" + err.message + "'");
		});
...
function displayUber(response, conFxn, prefix, postfix, files) {
	files = files || [];
	O.t('looking for files in ' + prefix + ' with names(' + files.length
			+ '): ' + files);
	var allData = {};
	...
	function awaitAll() {
		if (Object.keys(allData).length != files.length) {
			O.d('Waiting found sizes: ' + Object.keys(allData).length + ' and '
					+ files.length);
...
function displayFileByPath(filePath, response) {
	getDataFromFile(filePath, {
		...
		error : function() {
			O.e("File + '" + filePath
					+ "' threw error when a read was attempted");
			writeResponse(response, 500, con
					.text('Encountered an unknown error...'));
		},
		complete : function(data) {
			O.d("File + '" + filePath + "' was found and read...");
			writeResponse(response, 200, getContentFromFilePathAndData(
					filePath, data));
		}
	});
}
...
function simpleFileBasedWebServer(request, response) {
	...
	var filePath = __webappRoot
			+ ((urlPath == '/') ? '/html/index.html' : urlPath);

	if (filePath.endsWith('uber-js.js')) {
		O.d('Trying to display uber-js.js');
		if (query) {
			displayUber(response, con.js, __webappRoot + '/js/uber/', '.js',
					(query.files || '').split(','));
		} else {
		...
		if (query) {
			displayUber(response, con.css, __webappRoot + '/css/uber/', '.css',
					(query.files || '').split(','));
		} else {
...
/*******************************************************************************
 * @region diffusion-request
 * 
 * All Diffusion specific requests
 * 
 ******************************************************************************/

/**
 * Handle a /Diffusion/* request.
 * 
 * @param request =>
 *            http request object
 * @param response =>
 *            http response object
 */
function diffusionRequest(request, response) {
	var urlObj = url.parse(request.url, true);
	var urlPath = urlObj.pathname || '/';
	if (urlPath.length > 1) {
		...
		O.i("Executing '" + mthd + "'");
		try {
			diffusionFxns[mthd](urlObj.query, getDiffusionCallbacks({
				'success' : function(resp) {
					if (!resp.value) {
						resp = con.text('[SUCCESS] No content returned');
					}
					O.i('[Diffusion/' + mthd + ' SUCCESS] ' + resp.value);
				...
				'failure' : function(resp) {
					...
					O.w('[Diffusion/' + mthd + ' FAILURE] ' + resp.value);
				...
				'error' : function(resp) {
					...
					O.e('[Diffusion/' + mthd + ' ERROR] ' + resp.value);
					writeResponse(response, 500, resp);
				}
			}));
		} catch (err) {
 ...
 */
var diffusionFxnCallback = {
	'success' : function(resp) {
		O.i('[DEFAULT DIFFUSION CALLBACK] [SUCCESS] ' + resp.value);
	},
	'failure' : function(resp) {
		O.w('[DEFAULT DIFFUSION CALLBACK] [FAILURE] ' + resp.value);
	},
	'error' : function(resp) {
		O.e('[DEFAULT DIFFUSION CALLBACK] [ERROR] ' + resp.value);
	}
};

/**
 * Uses the <b><i>diffusionFxnCallback</i></b> as the base, replace each
 * default with the given callback implementation (if it exists)
 * 
 * @param callback =>
 *            override for <b><i>diffusionFxnCallback</i></b>
 * @returns callback object that is the merged result
 * @see diffusionFxnCallback
 */
function getDiffusionCallbacks(callback) {
	var tmp = {};
	for ( var attrname in diffusionFxnCallback) {
		tmp[attrname] = diffusionFxnCallback[attrname];
	}
...
/**
 * All the function Diffusion uses
 * 
 * Each takes 2 params:
 * 
 * @param args =>
 ...
 * 
 * @see getDiffusionCallbacks(callback)
 * @see diffusionFxnCallback
 */
var diffusionFxns = {
	/**
	...
	/**
	 * Checks the status of the diffusion connection
	 * 
	 * @param args
	 * @param callback
	 */
	'status' : function(args, callback) {
		callback.success(con
				.text((__diffusion) ? ('Is connected: ' + __diffusion
						.isConnected()) : 'Diffusion is null or not defined'));
	},
	/**
	...
	/**
	 * Returns the connection information, so the server can be the source for
	 * the app
	 * 
	 * @param args
	 * @param callback
	 */
	'connectionInfo' : function(args, callback) {
		callback.success(con.json('{"host" : "' + __diffHost + '", "port" : "'
				+ __diffPort + '", "ssl" : "' + __diffSSL + '"}'));
	},
	/**
	 * Adds a user to Diffusion instance
	 * 
	 * @param args
	 * @param callback
	 */
	'addUser' : function(args, callback) {
		O.i("Request to add user : '" + args.principal + "'");
		if (args.principal && args.password) {
			__diffusion.security.updateSystemAuthentication(
					__diffusion.security.createScriptBuilder().addPrincipal(
							args.principal, args.password,
							[ 'TOPIC_CONTROL', 'CLIENT' ]).script())
					.on(
							{
								complete : function() {
									callback.success(con
											.text('Creation of principal \''
													+ args.principal
													+ '\' complete.'));
								},
								rejected : function(reasons) {
									var tmpStr = 'Rejected.  Reasons: [';
									...
									tmpStr = tmpStr.replace(new RegExp(', $'),
											']');
									callback.failure(con.text(tmpStr));
								},
								error : function(err) {
									callback.error(con.text('Error:: ' + err));
								}
							});
		} else {
			callback.error(con
					.text('Must provide valid principal and password'));
		}
	},
	/**
	 * Checks to see that a given user exists (or doesn't)
	 * 
	 * @param args
	 * @param callback
	 */
	'hasUser' : function(args, callback) {
		O.i("Request to hasUser : '" + args.principal + "'");
		if (args.principal) {
			__diffusion.security
					.getSystemAuthentication({
						error : function(err) {
							callback
									.error(con
											.text('Error getting system authentication: '
													+ err));
						},
						complete : function(config) {
							var prinCount = config.principals.filter(function(
									principal) {
								return principal.name == args.principal;
							}).length;
							callback.success(con.text('Found: ' + prinCount
									+ ' with principal name \''
									+ args.principal + '\''));
						}
					});
		} else {
			callback.error(con.text('Must provide valid principal'));
		}
	}
